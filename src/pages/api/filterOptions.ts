...
export default async function handler(req: any, res: any) {
    try {
        // to query distinct values [return 'filter options'] then return unique values 
        ...
        // res: JSON object with unique values for each filter option (language, country, etc...)
        res.status(200).json({
    ...
    // catch error on issue when querying the database
    } catch (error) {
